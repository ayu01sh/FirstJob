...
async def upload_resume(
    ...
    jobs = await get_db().jobs.find({}, {"skills_required": 1}).to_list(length=500)
    seeded_skills = {s.lower() for job in jobs for s in job.get("skills_required", [])}
    role = target_role or current_user.get("profile", {}).get("target_role", "")
